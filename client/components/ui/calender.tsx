...
interface CalendarProps extends VariantProps<typeof calendarVariants> {
  ...
  alwaysOnTop?: boolean;
  /** Optional render function for day cells — use to overlay dots, badges, etc. */
  renderDay?: (day: number, date: Date) => React.ReactNode;
  /** Called when the visible month changes (via arrow navigation or pickers). */
...
function Calendar({
  ...
  /** Render a single day button, delegating to renderDay if provided */
  const renderDayButton = (day: number, monthOffset: number, keyPrefix: string) => {
    const date = new Date(currentYear, currentMonth + monthOffset, day);
    return (
      <button
        key={`${keyPrefix}-${day}`}
        ...
        className={cn(
          dayVariants({ variant: getDayVariant(day, monthOffset), size }),
          "relative"
        )}
        disabled={isDateDisabled(date)}
      >
        {renderDay ? renderDay(day, date) : day}
      </button>
    );
  };
      ...
      <div className="relative overflow-hidden">
        <AnimatePresence mode="wait" custom={direction}>
          ...
          >
            {/* Previous month days */}
            {showOutsideDays &&
              prevMonthDays.map((day) => renderDayButton(day, -1, "prev"))}

            {/* Current month days */}
            {currentMonthDays.map((day) => renderDayButton(day, 0, "current"))}

            {/* Next month days */}
            {showOutsideDays &&
              nextMonthDays.map((day) => renderDayButton(day, 1, "next"))}
          </motion.div>
        </AnimatePresence>
      </div>
    </div>
...
export { Calendar, calendarVariants, dayVariants, isSameDay, type CalendarProps };
